...
<p align="center">
  <a href="https://github.com/GlassHaven/Haven/releases/latest"><img src="https://img.shields.io/badge/release-v5.55.0-blue?style=flat-square" alt="Release" /></a>
...
- **Connections** — Host-key TOFU, port forwarding (-L/-R/-D/-J), SOCKS/HTTP proxies, Tor, ProxyJump, **per-app WireGuard, Tailscale, and NetBird tunnels** (userspace, no system VPN slot — each profile can route through its own tunnel without affecting other apps), and **port knocking / Single Packet Authorization** — fire a TCP/UDP knock sequence, or send a native fwknop SPA packet (AES-256-CBC + HMAC-SHA256, interoperable with `fwknopd`), at the remote firewall before the real connect, with per-profile fields and in-dialog "Test knock" / "Test SPA" buttons.
